...
int64_t hook(uint32_t reserved) {
    ...
    int64_t num_sellers = 2;

    // ACCOUNT: Hook Account
    uint8_t hook_acc[20];
    hook_account(HOOK_ACC, 20);

    uint8_t otxn_param_amt[8];
    int64_t otxn_param_size = otxn_param(otxn_param_amt, 8, "S1P", 3);
    // uint64_t seller_one_amount = float_int(*((int64_t*)otxn_param_amt), 0, 1);
    // TRACEVAR(seller_one_amount);

    uint8_t otxn_param_acct[20];
    int64_t otxn_param_size_1 = otxn_param(otxn_param_acct, 20, "S1A", 3);
    TRACEHEX(otxn_param_acct);

    etxn_reserve(num_sellers);

    for (int i = 0; GUARD(2), i < num_sellers; ++i)
    {

        ACCOUNT_TO_BUF(OTX_ACC, otxn_param_acct);
        ...
        *((uint32_t *)(LLS_OUT)) = FLIP_ENDIAN(lls);
        ...
        TRACEHEX(txn_currency);
        TRACEHEX(txn_issuer);
        TRACEHEX(otxn_param_amt);
        uint64_t amount = FLIP_ENDIAN_64(UINT64_FROM_BUF(otxn_param_amt));
